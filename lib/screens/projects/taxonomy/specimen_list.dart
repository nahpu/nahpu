                            ...
                            children: [
                              ...
                              SpecimenList(
                                data: _isSearching
                                    ...
                                    : specimenData,
                                additionalHeight: _isSearchOptionVisible
                                    ? 0
                                    : 86,
                              ),
                            ],
        ...
        child: ListView.builder(
          ...
          itemBuilder: (context, index) {
            return ListTile(
              ...
              subtitle: SpecimenListSubtitle(data: data[index]),
              ...
              onTap: () {
                Navigator.push(
                  ...
                  MaterialPageRoute(
                    builder: (context) =>
                        SpecimenFormView(specimenUuid: data[index].uuid),
                  ),
                );
              },
            );
...
class SpecimenListTitle extends ConsumerWidget {
  ...
  Future<String> _getPersonnelName(String? catalogerID, WidgetRef ref) async {
    if (catalogerID != null) {
      PersonnelData data = await PersonnelServices(
        ref: ref,
      ).getPersonnelByUuid(catalogerID);
      return data.name ?? '';
    } else {
      return '';
    }
...
class SpecimenListSubtitle extends ConsumerWidget {
  const SpecimenListSubtitle({super.key, required this.data});
  ...
  Future<String> _getTaxonData(int? taxonId, WidgetRef ref) async {
    if (taxonId != null) {
      TaxonomyData data = await TaxonomyServices(
        ref: ref,
  ...
  String _createTaxonInfo(TaxonomyData data) {
    String order = data.taxonOrder != null
        ? '${data.taxonOrder}$listTileSeparator'
        : '';
    String family = data.taxonFamily != null
        ? '${data.taxonFamily}$listTileSeparator'
        : '';
