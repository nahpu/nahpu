...
class PersonnelQuery extends DatabaseAccessor<Database>
  ...
  Future<bool> isImageUsed(String baseName) async {
    final PersonnelData? personnelList =
        await (select(personnel)
  ...
  Future<bool> isPersonnelUsedBySpecimenRecords({
    ...
    SpecimenData? specimenRecords =
        await (select(specimen)
              ..where((t) => t.projectUuid.equals(projectUuid))
              ..where(
                (t) =>
                    t.catalogerID.equals(personnelUuid) |
                    t.determinerID.equals(personnelUuid) |
  ...
  Future<bool> isPersonnelUsedByCollEvents({
    ...
    List<CollEventData> eventRecords =
        await (select(collEvent)
              ..where((t) => t.projectUuid.equals(projectUuid))
              ..limit(1))
            .get();
    ...
    for (final record in eventRecords) {
      CollPersonnelData? person =
          await (select(collPersonnel)..where(
                (tbl) =>
  ...
  Future<void> deleteProjectPersonnel({
    ...
    return (delete(personnelList)..where(
          (t) =>
  ...
  Future<List<PersonnelData>> getPersonnelByProjectUuid(
    ...
    List<PersonnelListData> personnelByProject = await (select(
      personnelList,
    )..where((t) => t.projectUuid.equals(projectUuid))).get();
