...
void main() {
  ...
  test('species and family counts are grouped and project scoped', () async {
    ...
    expect(
      species.map((row) => (row.label, row.count)),
      [
        ('Myotis lucifugus', 2),
        ('Eptesicus fuscus', 1),
    ...
    expect(
      families.map((row) => (row.label, row.count)),
      [('Vespertilionidae', 3), ('No family', 1)],
    );
  });
  ...
  test('part statistics sum quantities and normalize missing values', () async {
    ...
    expect(
      types.map((row) => (row.label, row.count)),
      [('Wing', 5), ('No part type', 1)],
    );
    expect(
      treatments.map((row) => (row.label, row.count)),
      [('No treatment', 4), ('Frozen', 2)],
    );
  });
  ...
  test('site and species filters aggregate only matching records', () async {
    ...
    final siteRows = await query
        ...
        .first;
    final myotis =
        taxa.singleWhere((option) => option.label.startsWith('Myotis'));
  ...
  test('record totals are calculated in SQLite', () async {
    ...
    expect(totals.speciesCount, 3);
    expect(totals.familyCount, 2);
  });
...
Future<void> _seedStatistics(Database db) async {
  ...
  final site = await db.into(db.site).insert(
        const SiteCompanion(
  ...
  for (var index = 0; index < 6; index++) {
    final taxon = await db.into(db.taxonomy).insert(
          TaxonomyCompanion(
